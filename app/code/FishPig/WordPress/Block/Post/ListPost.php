...
class ListPost extends \FishPig\WordPress\Block\Post
{
	...
	/**
	 * Returns the collection of posts for homepage
	 *
	 * @return Fishpig_Wordpress_Model_Mysql4_Post_Collection
	 */
	public function getHomepagePosts()
	{
		if ($this->_postCollection === null) {
			...
			$baseLimit = 4;
			$this->_postCollection = $this->_factory->getFactory('Post')->create()->getCollection();
			$this->_postCollection->addIsViewableFilter()->addOrder('post_date', 'desc')
					->addPostTypeFilter('post')
					->setPageSize($baseLimit);
			
			$this->_postCollection->setFlag('after_load_event_block', $this);
