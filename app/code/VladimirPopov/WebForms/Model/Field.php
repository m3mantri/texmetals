...
class Field extends AbstractModel implements IdentityInterface
{
    ...
    public function getFieldTypes()
    {
        $types = new \Magento\Framework\DataObject(array(
            ...
            "select/product" => __('Select / Limit Order Product'),
            "select/sellback-product" => __('Select / Sell Back Product'),
    ...
    public function toHtml()
    {
        ...
        switch ($field_type) {
            ...
            case 'select/product':
                ...
                $collection->addAttributeToFilter('limitorderable', 1);

                $config['products'] = $collection;
                $config['template'] = 'select_limitorder_product.phtml';
                break;
            case 'select/sellback-product':
                if (empty($config['field_value']) && !$config['customer_value']) {
                    $customer = $this->getCustomer();
                    if ($customer) {
                        $address = $customer->getDefaultBillingAddress();
                ...
                $collection->addAttributeToFilter('status', ['in' => $this->_productStatus->getVisibleStatusIds()]);
                $collection->setVisibility($this->_productVisibility->getVisibleInSiteIds());
                $collection->addAttributeToFilter('sellback', 1);

                $config['products'] = $collection;
                $config['template'] = 'select_sellback_product.phtml';
                break;
