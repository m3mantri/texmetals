...
class PrintInvoiceButton extends SplitButton
{
    protected function _construct()
    {
        ...
        if ( $entityId )
            $this->setOnclick("setLocation('" . $this->getUrl('snmpdfprint/invoice/print',['invoice_id'=>$entityId]) . "')");
    ...
    {
        if ( !$this->_scopeConfig->isSetFlag('snmportal_pdfprint/invoice/enabled')
    ...
    {
        return  $this->_coreRegistry->registry('current_invoice')? $this->_coreRegistry->registry('current_invoice')->getEntityId():0;
    }
        ...
        if ( $document )
        {
            ...
            foreach ( $this->_pdfHelper->getTemplatesForDocument($document) as $template )
            {
                $options[] = [
                    ...
                    'onclick' =>
                        "setLocation('" . $this->getUrl('snmpdfprint/invoice/print',['invoice_id'=>$document->getEntityId(), 'tplid'=>$template->getId()])."')"
                ];
            }
        }
        return $options;
