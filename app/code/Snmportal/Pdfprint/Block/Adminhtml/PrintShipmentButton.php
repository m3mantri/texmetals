...
class PrintShipmentButton extends SplitButton
{
    protected function _construct()
    {
        ...
        if ( $entityId )
            $this->setOnclick("setLocation('" . $this->getUrl('snmpdfprint/shipment/print',['shipment_id'=>$entityId]) . "')");
    ...
    protected function _getDefaultCss()
    {
        ...
        if ( !$this->_scopeConfig->isSetFlag('snmportal_pdfprint/shipment/show_default_print_button')
    ...
    {
        return  $this->_coreRegistry->registry('current_shipment')? $this->_coreRegistry->registry('current_shipment')->getEntityId():0;
    }
        ...
        if ( $document )
        {
            ...
            foreach ( $this->_pdfHelper->getTemplatesForDocument($document) as $template )
            {
                $options[] = [
                    ...
                    'onclick' =>
                        "setLocation('" . $this->getUrl('snmpdfprint/shipment/print',['shipment_id'=>$document->getEntityId(), 'tplid'=>$template->getId()])."')"
                ];
            }
        }
        return $options;
