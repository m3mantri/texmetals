...
class PrintCreditmemoButton extends SplitButton
{
    protected function _construct()
    {
        ...
        if ( $entityId )
            $this->setOnclick("setLocation('" . $this->getUrl('snmpdfprint/creditmemo/print',['creditmemo_id'=>$entityId]) . "')");
    ...
    {
        if ( !$this->_scopeConfig->isSetFlag('snmportal_pdfprint/creditmemo/enabled')
    ...
    {
        return  $this->_coreRegistry->registry('current_creditmemo')? $this->_coreRegistry->registry('current_creditmemo')->getEntityId():0;
    }
        ...
        if ( $document )
        {
            ...
            foreach ( $this->_pdfHelper->getTemplatesForDocument($document) as $template )
            {
                $options[] = [
                    ...
                    'onclick' =>
                        "setLocation('" . $this->getUrl('snmpdfprint/creditmemo/print',['creditmemo_id'=>$document->getEntityId(), 'tplid'=>$template->getId()])."')"
                ];
            }
        }
        return $options;
