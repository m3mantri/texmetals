...
class PrintOrderButton extends SplitButton
{
    protected function _construct()
    {
        ...
        if ( $entityId )
            $this->setOnclick("setLocation('" . $this->getUrl('snmpdfprint/order/print',['order_id'=>$entityId]) . "')");
    ...
    {
        if ( !$this->_scopeConfig->isSetFlag('snmportal_pdfprint/order/enabled')
    ...
    {
        return  $this->_coreRegistry->registry('current_order')? $this->_coreRegistry->registry('current_order')->getEntityId():0;
    }
        ...
        if ( $document )
        {
            ...
            foreach ( $this->_pdfHelper->getTemplatesForDocument($document) as $template )
            {
                $options[] = [
                    ...
                    'onclick' =>
                        "setLocation('" . $this->getUrl('snmpdfprint/order/print',['order_id'=>$document->getEntityId(), 'tplid'=>$template->getId()])."')"
                ];
            }
        }
        return $options;
