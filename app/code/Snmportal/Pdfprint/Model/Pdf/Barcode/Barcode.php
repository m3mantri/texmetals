    ...
    public function getBarcode2D($type,$code, $w = 3, $h = 3, $color = 'black')
    {
        ...
        switch ($qrtype) {
            case 'DATAMATRIX': { // DATAMATRIX (ISO/IEC 16022)
                require_once(__DIR__ . '/../../../Test/M1/Barcode/Datamatrix.php');
                $barcode = new \Snmportal\External\Barcode\Datamatrix($code);
            ...
            case 'PDF417': { // PDF417 (ISO/IEC 15438:2006)
                ...
                require_once(__DIR__ . '/../../../Test/M1/Barcode/PDF417.php');
                $barcode = new \Snmportal\External\Barcode\PDF417($code, $ecl, $aspectratio, $macro);
                $this->barcode_array = $barcode->getBarcodeArray();
                $this->barcode_array['code'] = $code;
                break;
            }
            case 'QRCODE': { // QR-CODE
                require_once(__DIR__ . '/../../../Test/M1/Barcode/QRcode.php');
                if (!isset($mode[1]) OR (!in_array($mode[1], array('L', 'M', 'Q', 'H')))) {
                    $mode[1] = 'L'; // Ddefault: Low error correction
                }
                $barcode = new \Snmportal\External\Barcode\QRcode($code, strtoupper($mode[1]));
                $this->barcode_array = $barcode->getBarcodeArray();
                $this->barcode_array['code'] = $code;
                break;
            }
